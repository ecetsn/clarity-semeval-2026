# src/representation/scripts/run_yamls_roberta_opt_projection_depths.py
#
# Runs train_from_yaml sequentially on YAMLs under:
#   <project_root>/src/representation/configs/roberta-opt/projection_ablation/
# selecting only files matching:
#   clarity_roberta_opt_l*.yaml
#
# Runner stdout/stderr logs + JSONL summary are written to:
#   <project_root>/experiments/logs/run_logs/roberta_opt_project_ablations/
#
# Usage:
#   python src/representation/scripts/run_yamls_roberta_opt_projection_depths.py
...
def main() -> None:
    ...
    # Only clarity_roberta_opt_l*.yaml
    yamls = sorted(yamls_dir.glob("clarity_roberta_opt_l*.yaml"))
    if len(yamls) == 0:
        raise FileNotFoundError(f"No matching YAML files found in: {yamls_dir} (pattern: clarity_roberta_opt_l*.yaml)")

    # Runner logs (stdout/stderr) + summary JSONL
    run_logs_dir = repo_root / "experiments" / "logs" / "run_logs" / "roberta_opt_project_ablations"
    run_logs_dir.mkdir(parents=True, exist_ok=True)
    summary_path = run_logs_dir / "clarity_roberta_opt_projection_ablations.jsonl"
