# src/representation/scripts/run_yamls_model_scale.py
#
# Runs train_from_yaml sequentially on all YAMLs under:
#   <project_root>/src/representation/configs/model-scale/*.yaml
#
# Runner stdout/stderr logs + JSONL summary are written to:
#   <project_root>/experiments/logs/run_logs/model-scale/
#
# Usage:
#   python src/representation/scripts/run_yamls_model_scale.py
...
def main() -> None:
    ...
    yamls_dir = repo_root / "src" / "representation" / "configs" / "model-scale"
    ...
    run_logs_dir = repo_root / "experiments" / "logs" / "run_logs" / "model-scale"
    run_logs_dir.mkdir(parents=True, exist_ok=True)
    summary_path = run_logs_dir / "model_scale_runs.jsonl"
